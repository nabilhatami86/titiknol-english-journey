import type { ModuleLesson } from "@/types/module";

// ── Intermediate 2 Reading Lessons ───────────────────────────────────────────

export const intermediate2ReadingLessons: ModuleLesson[] = [
  // ── Day 2 · The Power of Language and Communication ──────────────────────────
  {
    id: "int2-reading-2",
    track: "reading",
    day: 2,
    tutor: "Mrs. Aji",
    title: "The Power of Language and Communication",
    subtitle:
      "How language shapes thinking, identity, and professional success",
    overview:
      "Bacaan akademik tentang peran bahasa dalam kehidupan manusia: mulai dari pembentukan kognitif, komunikasi non-verbal, hingga dampak kompetensi bahasa terhadap dunia profesional dan pertumbuhan ekonomi.",
    passage: [
      "Language is one of the most powerful tools that humans possess. It is not merely a system of communication, but a vehicle through which ideas, emotions, culture, and identity are expressed and preserved. The ability to articulate thoughts clearly and persuasively distinguishes effective communicators from average ones. In today's globalized world, individuals who can navigate multiple languages and communication styles hold a significant advantage, both professionally and socially.",

      "(Bahasa adalah salah satu alat paling kuat yang dimiliki manusia. Bahasa bukan sekadar sistem komunikasi, melainkan sebuah sarana untuk mengekspresikan dan melestarikan gagasan, emosi, budaya, dan identitas. Kemampuan untuk mengartikulasikan pikiran secara jelas dan persuasif membedakan komunikator yang efektif dari yang biasa-biasa saja. Di dunia yang telah mengglobal seperti saat ini, individu yang mampu menavigasi berbagai bahasa dan gaya komunikasi memiliki keunggulan yang signifikan, baik secara profesional maupun sosial.)",

      "Research in linguistics has consistently shown that learning a second language does more than expand one's vocabulary. It fundamentally reshapes the brain's cognitive architecture. Bilingual individuals have been found to exhibit greater mental flexibility, enhanced problem-solving abilities, and a stronger capacity to filter out irrelevant information. Moreover, the process of acquiring a new language forces learners to examine their own native tongue more critically, fostering a deeper appreciation of linguistic nuance and cultural diversity.",

      "(Penelitian dalam bidang linguistik secara konsisten menunjukkan bahwa mempelajari bahasa kedua tidak hanya sekadar memperluas kosakata seseorang. Proses ini secara fundamental membentuk ulang arsitektur kognitif otak. Individu bilingual terbukti menunjukkan fleksibilitas mental yang lebih besar, kemampuan pemecahan masalah yang lebih baik, dan kapasitas yang lebih kuat untuk menyaring informasi yang tidak relevan. Lebih dari itu, proses belajar bahasa baru mendorong pelajar untuk mengkaji bahasa ibu mereka sendiri secara lebih kritis, sehingga menumbuhkan apresiasi yang lebih dalam terhadap nuansa linguistik dan keragaman budaya.)",

      "Communication, however, extends beyond spoken and written language. Non-verbal cues — such as facial expressions, gestures, posture, and tone of voice — constitute a substantial portion of the messages we convey. Studies suggest that in face-to-face interactions, non-verbal communication may account for as much as 70% of the total meaning transmitted. Therefore, mastering a language requires not only grammatical accuracy and vocabulary breadth, but also a keen awareness of cultural norms and interpersonal dynamics.",

      "(Namun, komunikasi tidak hanya terbatas pada bahasa lisan dan tulisan. Isyarat non-verbal — seperti ekspresi wajah, gerak tubuh, postur, dan nada suara — merupakan bagian yang signifikan dari pesan yang kita sampaikan. Penelitian menunjukkan bahwa dalam interaksi tatap muka, komunikasi non-verbal dapat menyumbang hingga 70% dari total makna yang disampaikan. Oleh karena itu, menguasai suatu bahasa tidak hanya memerlukan ketepatan tata bahasa dan kekayaan kosakata, tetapi juga kesadaran yang tajam terhadap norma budaya dan dinamika antarpribadi.)",

      "In professional settings, the ability to communicate effectively is widely regarded as the most critical skill an individual can develop. Companies across the globe increasingly seek employees who can present ideas coherently, collaborate across cultural boundaries, and adapt their communication style to diverse audiences. Consequently, investment in language education has grown substantially in developing nations, as governments and private institutions recognize that linguistic competency is inseparable from economic growth and global competitiveness.",

      "(Dalam lingkungan profesional, kemampuan berkomunikasi secara efektif secara luas dianggap sebagai keterampilan paling kritis yang dapat dikembangkan seseorang. Perusahaan di seluruh dunia semakin mencari karyawan yang mampu menyajikan ide secara koheren, berkolaborasi lintas batas budaya, dan menyesuaikan gaya komunikasi mereka untuk audiens yang beragam. Akibatnya, investasi dalam pendidikan bahasa telah berkembang secara signifikan di negara-negara berkembang, karena pemerintah dan institusi swasta menyadari bahwa kompetensi linguistik tidak dapat dipisahkan dari pertumbuhan ekonomi dan daya saing global.)",

      "Despite its undeniable importance, language learning is not without its challenges. Many learners experience anxiety, fear of making mistakes, and a reluctance to practice with native speakers. Educators and researchers agree that creating a supportive and low-anxiety environment is essential to accelerating language acquisition. When learners feel safe to experiment with the language, make errors, and receive constructive feedback, their progress tends to be significantly more rapid and sustained. In this regard, the journey of language learning is as much about cultivating a resilient mindset as it is about memorizing grammar rules and vocabulary lists.",

      "(Terlepas dari pentingnya yang tak terbantahkan, pembelajaran bahasa tidak luput dari tantangan. Banyak pelajar mengalami kecemasan, rasa takut membuat kesalahan, dan keengganan untuk berlatih dengan penutur asli. Para pendidik dan peneliti sepakat bahwa menciptakan lingkungan yang suportif dan minim tekanan sangat penting untuk mempercepat penguasaan bahasa. Ketika pelajar merasa aman untuk bereksperimen dengan bahasa, membuat kesalahan, dan menerima umpan balik yang konstruktif, kemajuan mereka cenderung jauh lebih pesat dan berkelanjutan. Dalam hal ini, perjalanan belajar bahasa sama-sama tentang menumbuhkan pola pikir yang tangguh seperti halnya menghafal aturan tata bahasa dan daftar kosakata.)",
    ],
    materialSections: [
      {
        title: "Main Passage Focus",
        points: [
          "Para. 1 — Language = alat terkuat manusia: menyampaikan ide, emosi, budaya, dan identitas.",
          "Para. 2 — Belajar bahasa kedua membentuk ulang arsitektur kognitif otak; bilinguals lebih fleksibel dan kritis.",
          "Para. 3 — Komunikasi non-verbal (ekspresi, gerak tubuh, nada) = hingga 70% makna dalam interaksi tatap muka.",
          "Para. 4 — Di dunia profesional, komunikasi efektif = skill paling kritis; investasi pendidikan bahasa terus meningkat.",
          "Para. 5 — Tantangan belajar bahasa: kecemasan & takut salah; solusi = lingkungan suportif + pola pikir tangguh.",
        ],
      },
      {
        title: "Reading Strategy: Identifying Author's Purpose",
        points: [
          "Author's purpose = alasan penulis menulis teks (to inform, to persuade, to entertain).",
          "Teks ini bertujuan TO INFORM (menginformasikan) sekaligus TO PERSUADE (meyakinkan) pembaca.",
          "",
          "Kata-kata yang menunjukkan argument/persuasion:",
          "   'Research has consistently shown...'",
          "   '...is widely regarded as...'",
          "   '...is inseparable from...'",
          "",
          "Kata-kata yang menunjukkan cause-effect:",
          "   'Consequently...' | 'Therefore...' | 'Moreover...'",
          "   'As a result...' | 'In this regard...'",
          "",
          "Tip: Paragraf terakhir sering berisi solusi/rekomendasi penulis.",
          "   Di sini: 'creating a supportive environment is essential to accelerating language acquisition.'",
        ],
      },
      {
        title: "Key Vocabulary",
        points: [
          "possess, vehicle, articulate, persuasively, navigate",
          "linguistics, cognitive architecture, bilingual, flexibility, irrelevant",
          "non-verbal cues, gestures, posture, constitute, convey",
          "coherently, collaborate, boundaries, competency, inseparable",
          "anxiety, reluctance, acquisition, constructive, resilient",
        ],
      },
      {
        title: "Vocabulary List",
        points: [
          "1.  possess (v) — memiliki | Sinonim: own, have | Antonim: lack",
          "2.  vehicle (n) — sarana/kendaraan | Sinonim: medium, means | Antonim: obstacle",
          "3.  articulate (v) — mengartikulasikan/mengungkapkan | Sinonim: express, convey | Antonim: mumble",
          "4.  persuasively (adv) — secara persuasif | Sinonim: convincingly | Antonim: unconvincingly",
          "5.  navigate (v) — menavigasi/melewati | Sinonim: manage, handle | Antonim: avoid",
          "6.  linguistics (n) — linguistik (ilmu bahasa) | Sinonim: language science",
          "7.  cognitive (adj) — kognitif (berkaitan dengan pikiran) | Sinonim: mental, intellectual | Antonim: emotional",
          "8.  architecture (n) — arsitektur/struktur | Sinonim: framework, structure | Antonim: chaos",
          "9.  bilingual (adj) — bilingual/dwibahasa | Sinonim: two-language speaker | Antonim: monolingual",
          "10. flexibility (n) — fleksibilitas | Sinonim: adaptability | Antonim: rigidity",
          "11. irrelevant (adj) — tidak relevan | Sinonim: unrelated | Antonim: relevant",
          "12. foster (v) — menumbuhkan/mendorong | Sinonim: nurture, promote | Antonim: hinder",
          "13. nuance (n) — nuansa/perbedaan halus | Sinonim: subtlety | Antonim: bluntness",
          "14. non-verbal (adj) — non-verbal/tanpa kata | Sinonim: unspoken | Antonim: verbal",
          "15. cues (n) — isyarat/petunjuk | Sinonim: signals, hints | Antonim: masks",
          "16. gestures (n) — gerak tubuh | Sinonim: body language | Antonim: stillness",
          "17. posture (n) — postur/sikap tubuh | Sinonim: stance, bearing",
          "18. constitute (v) — membentuk/merupakan | Sinonim: make up, form | Antonim: exclude",
          "19. convey (v) — menyampaikan | Sinonim: communicate, express | Antonim: withhold",
          "20. transmitted (v) — dikirimkan/disampaikan | Sinonim: conveyed, sent | Antonim: withheld",
          "21. breadth (n) — keluasan/cakupan | Sinonim: scope, range | Antonim: narrowness",
          "22. keen (adj) — tajam/antusias | Sinonim: sharp, enthusiastic | Antonim: indifferent",
          "23. interpersonal (adj) — antarpribadi/antarpersonal | Sinonim: social, relational",
          "24. coherently (adv) — secara koheren/sistematis | Sinonim: logically, clearly | Antonim: incoherently",
          "25. collaborate (v) — berkolaborasi | Sinonim: cooperate, work together | Antonim: compete",
          "26. boundaries (n) — batasan/perbatasan | Sinonim: borders, limits | Antonim: freedom",
          "27. adapt (v) — menyesuaikan | Sinonim: adjust, modify | Antonim: resist",
          "28. substantially (adv) — secara substansial/signifikan | Sinonim: considerably | Antonim: slightly",
          "29. competency (n) — kompetensi/kemampuan | Sinonim: ability, skill | Antonim: incompetence",
          "30. inseparable (adj) — tidak dapat dipisahkan | Sinonim: inextricable | Antonim: separable",
          "31. competitiveness (n) — daya saing | Sinonim: rivalry | Antonim: cooperation",
          "32. undeniable (adj) — tak terbantahkan | Sinonim: indisputable | Antonim: debatable",
          "33. anxiety (n) — kecemasan | Sinonim: worry, nervousness | Antonim: calmness",
          "34. reluctance (n) — keengganan | Sinonim: unwillingness | Antonim: willingness",
          "35. acquisition (n) — penguasaan/perolehan | Sinonim: learning, attainment | Antonim: loss",
          "36. accelerate (v) — mempercepat | Sinonim: hasten, speed up | Antonim: slow down",
          "37. constructive (adj) — konstruktif/membangun | Sinonim: helpful, positive | Antonim: destructive",
          "38. sustained (adj) — berkelanjutan | Sinonim: continuous, ongoing | Antonim: temporary",
          "39. cultivate (v) — menumbuhkan/mengembangkan | Sinonim: develop, nurture | Antonim: neglect",
          "40. resilient (adj) — tangguh/ulet | Sinonim: tough, adaptable | Antonim: fragile",
        ],
      },
    ],
    exercises: [
      {
        id: "adv-r2-mc1",
        type: "multiple-choice",
        question: "What is the main idea of the first paragraph?",
        options: [
          "Language is only useful for professional communication",
          "Language is a powerful tool that expresses ideas, culture, and identity",
          "Multilingual people are more successful than monolingual people",
        ],
        correctAnswer:
          "Language is a powerful tool that expresses ideas, culture, and identity",
        reason:
          'Para. 1: "Language is one of the most powerful tools that humans possess... a vehicle through which ideas, emotions, culture, and identity are expressed and preserved."',
      },
      {
        id: "adv-r2-mc2",
        type: "multiple-choice",
        question:
          "According to the passage, what does learning a second language do to the brain?",
        options: [
          "It only expands the vocabulary",
          "It fundamentally reshapes the brain's cognitive architecture",
          "It makes people forget their native language",
        ],
        correctAnswer:
          "It fundamentally reshapes the brain's cognitive architecture",
        reason:
          'Para. 2: "It fundamentally reshapes the brain\'s cognitive architecture."',
      },
      {
        id: "adv-r2-mc3",
        type: "multiple-choice",
        question:
          "Based on the passage, how much of the total meaning in face-to-face interaction may non-verbal communication account for?",
        options: ["Up to 50%", "Up to 60%", "Up to 70%"],
        correctAnswer: "Up to 70%",
        reason:
          'Para. 3: "non-verbal communication may account for as much as 70% of the total meaning transmitted."',
      },
      {
        id: "adv-r2-mc4",
        type: "multiple-choice",
        question: "What is the main idea of the fourth paragraph?",
        options: [
          "Non-verbal communication is more important than verbal",
          "Effective communication is the most critical skill in professional life",
          "Language education is only important for developing countries",
        ],
        correctAnswer:
          "Effective communication is the most critical skill in professional life",
        reason:
          'Para. 4: "the ability to communicate effectively is widely regarded as the most critical skill an individual can develop."',
      },
      {
        id: "adv-r2-mc5",
        type: "multiple-choice",
        question:
          "What do educators and researchers agree is essential to accelerating language acquisition?",
        options: [
          "Strict grammar drills and memorization",
          "Learning with native speakers only",
          "Creating a supportive and low-anxiety environment",
        ],
        correctAnswer: "Creating a supportive and low-anxiety environment",
        reason:
          'Para. 5: "creating a supportive and low-anxiety environment is essential to accelerating language acquisition."',
      },
      {
        id: "adv-r2-mc6",
        type: "multiple-choice",
        question:
          "The word 'resilient' in the last paragraph is closest in meaning to...",
        options: ["fragile", "tough and adaptable", "passive"],
        correctAnswer: "tough and adaptable",
        reason:
          "'Resilient' = tangguh/ulet — the ability to recover and adapt from difficulties. Sinonim: tough, adaptable.",
      },
      {
        id: "adv-r2-mc7",
        type: "multiple-choice",
        question:
          "What can be inferred from the passage about bilingual individuals?",
        options: [
          "They tend to have better problem-solving abilities than monolingual people",
          "They always struggle with their native language",
          "They only benefit professionally, not socially",
        ],
        correctAnswer:
          "They tend to have better problem-solving abilities than monolingual people",
        reason:
          'Para. 2: "Bilingual individuals have been found to exhibit greater mental flexibility, enhanced problem-solving abilities..."',
      },
      {
        id: "adv-r2-sa1",
        type: "short-answer",
        question:
          "Why does the author say that mastering a language requires more than just grammar and vocabulary?",
      },
      {
        id: "adv-r2-sa2",
        type: "short-answer",
        question:
          "According to paragraph 4, why has investment in language education grown in developing nations?",
      },
      {
        id: "adv-r2-sa3",
        type: "short-answer",
        question:
          "What challenges do language learners typically face, according to the last paragraph?",
      },
      {
        id: "adv-r2-sa4",
        type: "short-answer",
        question:
          "What is the author's overall attitude toward language learning? Is it positive, negative, or balanced? Support with evidence from the text.",
      },
      {
        id: "adv-r2-task1",
        type: "task",
        section: "final",
        question:
          "SUMMARIZE the passage in 5–7 sentences using your own words. Make sure to cover the main idea of each paragraph.",
      },
      {
        id: "adv-r2-task2",
        type: "task",
        section: "final",
        question:
          "VOCABULARY TASK:\nPick 20 words from the vocabulary list that are new to you. Write a sentence for each word that shows you understand its meaning.",
      },
      {
        id: "adv-r2-task3",
        type: "task",
        section: "final",
        question:
          "REFLECTION: Based on the passage, what do you think is the most important reason to learn English? Write 3–5 sentences using at least 2 Noun Clauses (NC) from the grammar material (e.g. 'What I believe is...', 'The reason why I study English is that...')",
      },
    ],
  },
];
